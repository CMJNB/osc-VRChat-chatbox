...
- 默认源语言是中文，目标语言是英文。
- 输入框上方可直接切换源语言、目标语言和发送格式：原文 + 译文、仅译文、仅原文；双语格式可自定义为“原文 → 译文”或“译文 → 原文”。点击“设置”后可切换开机自启动、翻译开关和翻译服务/API 配置。
- 默认使用 MyMemory 免费公开翻译 API。
- 可填写 MyMemory email 提升免费额度，也可填写 MyMemory key 使用私有 TM/认证能力。
- 可选接入 OpenAI-compatible AI 大模型 API。
- AI 模式可使用系统默认提示词，也可保存、编辑、删除并切换最多 6 个自定义提示词模板；界面会显示保存状态，本地服务会校验并原子更新 `prompts.json`，重启或从其他设备访问同一服务后仍可使用。
- AI 模式提供共享术语词库，默认预置 30 个语言无关的 VRChat 保护词，包括 `VRChat`、`VRC`、`VCC`、`Udon`、`PhysBones`、`OSC` 和 `FBT` 等。默认词条参考 [VRChat 官方本地化指南](https://docs.vrchat.com/docs/suggesting-localization-changes)、[Creator Companion 术语表](https://vcc.docs.vrchat.com/guides/glossary/)、[VRChat Creation 文档](https://creators.vrchat.com/) 与 [OSC 文档](https://docs.vrchat.com/docs/osc-overview)筛选而成。每行可填写一个需要保护的术语，或使用 `原文 => 译文` 指定固定译法；词库与提示词一起保存在 `prompts.json`。
- AI 模式可开启“翻译前纠正原文”。模型会在一次请求中谨慎纠正明确的错别字、拼写或语法问题并翻译，优先匹配术语词库，同时避免把无关内容强行改成词库词语；“原文 + 译文”使用纠正后的原文。如果兼容 API 不按约定返回结构化结果，页面会自动退回普通翻译结果。
...
“翻译前纠正原文”和术语词库只用于 AI 提供商，不会对 MyMemory 生效。它们不会增加第二次 API 调用；页面会把词库作为最高优先级词汇数据，并在开启纠正时追加严格的 JSON 输出约定。词库支持保护词和 `原文 => 译文` 映射，纠正只会在输入可能是对应术语的拼写或语音识别变体时优先替换。页面兼容带 Markdown 代码块或退回纯译文的模型。纯数字、纯符号和 Emoji 等输入仍会在浏览器中直接短路，不调用任何翻译 API。
...
5. 输入框上方可以切换语言、发送格式和双语顺序；点击“设置”可以切换 UI 语言、翻译、翻译服务、API 配置、AI 术语词库、开机自启动和最小化自启动。
